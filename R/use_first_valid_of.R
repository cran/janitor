...
use_first_valid_of <- function(..., if_all_NA = NA) {
  ...
  if (length(unique(lapply(vars, length))) > 1) {
  ...
  if (!is.na(if_all_NA)) {
    if (class(if_all_NA) != class(result)) {
      stop("class(if_all_NA) does not match class of resulting vector")
    }
