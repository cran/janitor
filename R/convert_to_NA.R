...
convert_to_NA <- function(dat, strings) {
  ...
  if (!class(strings) %in% c("character", "numeric", "factor", "integer")) {
    stop("'strings' parameter should be a vector of class character, numeric, factor, or integer")
  }
  ...
  if (is.vector(dat) & !is.list(dat)) { # handle vector/list case, otherwise proceed for df
    ...
    stop("argument 'dat' must be a vector or data.frame")
  }
}
